...
class UpgradeIdps(object):
    ...
    def unzipUpgrade(self, srcPath):
        ...
        firstUnzipCmd = unzip_abs_path + " x -y -p" + self.configDict['unzip_pw'] + " -o" + firstDstPath + " " + srcPath
    ...
    def dbBackup(self):
        ...
        if not os.path.exists(today):
            ...
            if (os.mkdir(today)) == None:
                ...
                sys.exit()
        # 获取pg_dump工具的绝对路径
        pgdump_abs_path = os.path.abspath(".\\tools\\pgsql\\")
        print(pgdump_abs_path)
    ...
    def executeUpgradeSqlFile(self, unzipPath):
        ...
        print(sql_file_path)
        try:
            conn = psycopg2.connect(host=self.configDict['db_host'],
                                    port=self.configDict['db_port'],
                                    database=self.configDict['db_name'],
                                    user=self.configDict['db_user'],
                                    password=self.configDict['db_pw']
                                    )
            cursor = conn.cursor()
            with open(sql_file_path, encoding='utf-8', mode='r') as file:
                lines = file.readlines()
